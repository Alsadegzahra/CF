"""
Match registry + artifacts: SQLite for courts, matches, artifacts.
Uses config/settings.DB_PATH. Per-match tracks live in storage/tracks_db.py (tracks.db per match).
"""
...
def create_match(
    ...
    with connect() as conn:
        conn.execute(
            """
            INSERT INTO matches (match_id, court_id, source_type, source_uri, output_dir, state, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, 'CREATED', ?, ?)
            """,
...
def update_match(
    ...
    *,
    ...
    with connect() as conn:
        conn.execute(f"UPDATE matches SET {', '.join(fields)} WHERE match_id=?", tuple(values))
...
def list_matches(limit: int = 100) -> List[Dict[str, Any]]:
    with connect() as conn:
        rows = conn.execute("SELECT * FROM matches ORDER BY created_at DESC LIMIT ?", (limit,)).fetchall()
    return [dict(r) for r in rows]


def list_matches_by_state(state: str) -> List[Dict[str, Any]]:
    with connect() as conn:
        rows = conn.execute("SELECT * FROM matches WHERE state=? ORDER BY created_at ASC", (state,)).fetchall()
    return [dict(r) for r in rows]
...
def list_artifacts(match_id: str) -> List[Dict[str, Any]]:
    with connect() as conn:
        rows = conn.execute("SELECT * FROM artifacts WHERE match_id=? ORDER BY created_at DESC", (match_id,)).fetchall()
    return [dict(r) for r in rows]
